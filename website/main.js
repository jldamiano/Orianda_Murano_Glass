...
function stars (width){  //creating a function for the creation of the various stars, with width and height as parameters for each//
  class Star{
    ...
    create() {
      ...
      document.body.append(starImg); //appending to make the stars be visible//
    }
  }
  
  for(let i = 0; i < 50; i++){
    const size = Math.random()*80; //the amount of stars//
    let x = Math.random()*width; //setting parameter for width since it will change depending on responsive width
...
//creating function that delays the creation of the stars so that the appear haldway through the opening scene to make it appear like they stars are fading in once the image as reached full screen size//
function starStart() {
  setTimeout(function(){
    // If media query matches x size, then change the width of the space the stars shine on. this is because in mobile. i want the stars to take up the whole width, but only hald the image on desktop for better readability//
    function responsiveFunction(x) {  
      if (x.matches) { 
        stars(220); //passing through the numbers in the parametrs set in the function previously//
      } else {
        stars(620); //using inner height to capture 100vh//
      }
    }
...
// OPENING GSAP ANIMATION - initial gsap for the text entering as the screen loads//
...
function scrollyStuff(){   
  ...
  scrollTriggerBaseFunction(".tabbys",'.tab-titles', 0, 100, 0, 3, "power4)", 0.2, 0, '-=1');
  ...
  //INQUIRIES SCROLL TRIGGER//
  scrollTriggerBaseFunction(".inq",'.inq-title', 0, 300, 0, 2, "power4");
  scrollTriggerBaseFunction(".inq",'.inq-text', 0, 300, 0, 2, "power4", 0, 0, '-=1');
  scrollTriggerBaseFunction(".inq",'.form-side', 0, 500, 0, 2, "power4", 0, 0, '-=1');
  ...
  //Creating a function for all the cards since they all have the same trigger settings, just need to have their scroll starting position as separte from each other so they come up one at a time//
...
stick.addEventListener ('click', () => {
  ...
  opening.to('.fire-text', {  
    scale:0, //text zooms into the screen//
    duration: 2,
    ease: "power4",

    //using on complete function so that the next set of timelines play after the text zooms into the screen, effectivly opening up the rest of the site//
    onComplete: () => {
      ...
      starStart(); //calling the stars function so they can be created and fade into screen
...
//FLOATING NAVIGATION //
//creating a floating nav that only appears when the user scroll back up - code adapted from https://codepen.io/GreenSock/pen/qBawMGb (full reference in production document)//
...
function scrollynav (){ //creating function for scrolling navigation bar//
  ...
  ScrollTrigger.create({
    start: "top top", //making it start at the top of the page//
    end: 99999,
    onUpdate: (self) => {
      ...
      //changing the background colour of the nav bar depending on screen size so that contrast is enhanced to improve viewability//
...
//since clicking to inquire is independent to which item is displayed, using query selector all to grab all the close buttons, regardless of which dialog box it is in, and using for each to say that for every time each of the buttons is clicked, close all the dialog boxes and scroll to 'x'//
...
closeTest.forEach(function(closeTest){
  closeTest.addEventListener ('click', () => {  //when the close button is clicked, for each dialog box, close it//
    dialogBoxes.forEach(function(dialogBoxes){ //commanding all the boxes to be dialog boxes to be closed//
      ...
      document.querySelector(".inq").scrollIntoView({ block: 'end',  behavior: 'smooth' }); //scrolling to the inquiries section//
    });
});


//grabbing the images and the colour select cirlces for each image and all the showcased images. using their common class name to turn them into an array
...
//code from https://swiperjs.com/ - https://codesandbox.io/s/tylr6//
...
//DROP DOWN TABS//
//function that encompasses the animations for everything in the seal of certification tabs//
function dopIcons (){
  //creating a base function to set the basis of the gsap animiation, so that then it can just be applied to different divs/imgs in the section, passing through respective parameters//
  function dops (thing, op, yx, dur, eas, stag, sca, timing){
    const dopAppear = gsap.timeline();
    dopAppear.from(thing, {  
      opacity: op, 
      y: yx, 
      duration: dur, 
      ease: eas,
  ...
  dops ('.tab2-header', 0, -50, 2, "power4");
  dops ('.dop', 0, 0, 3, "back.out(1.4)", 1, 0, '-=1');
  dops ('.tab2-text', 0, 0, 1, "power4", 0, 0, '-=4');
}

//running the encompassing function once the tab is clicked open.
...
//timteline for third tab - it only two parts to the timeline so i didn;t make a base function as it was just a short one//
...
//ABOUT US SECTION//
// grabbing all the elements that can be clicked and the text boxes and turning them into arrays//  
...
const aboutScrollToLocation = document.querySelector('.b1');
const normalaboutScrollToLocation = document.querySelector('.frames');
...
//INPUT FORM SUBMIT//
//using gsap to fade in the alert message//
const thanksTl = gsap.timeline();
thanksTl.pause();

//using gsap to fade in the alert message//
thanksTl.to('.thankyou-message', {  
  opacity: 1,
  duration: 1,
  ease: "power4",
});

//using gsap to fade out the alert message//
const thanksBackTl = gsap.timeline();
thanksBackTl.pause();

thanksBackTl.to('.thankyou-message', {  
  opacity: 0,
  duration: 1,
  ease: "power4",
});
...
form.addEventListener('sl-submit', () => {
  ...
  thanksTl.play();
  thanks.scrollIntoView({ block: 'end',  behavior: 'smooth' });
});
...
//when the hamburger menu is clicked, change the class to active to make the hamburger turn into a cross - https://dev.to/devggaurav/let-s-build-a-responsive-navbar-and-hamburger-menu-using-html-css-and-javascript-4gci.//
...
const logoJump = document.querySelector('.nav-logo');
...
function hamburgerJumps (menuItem, jumpLocation){
  menuItem.addEventListener('click', () => {
    ...
    document.querySelector(jumpLocation).scrollIntoView({ block: 'end',  behavior: 'smooth' });
  });
}

//running function//
hamburgerJumps (productJump, "#product-table");
hamburgerJumps (processJump, "#process-caro");
hamburgerJumps (aboutJump, "#about-us");
hamburgerJumps (inqJump, "#inquiries");
hamburgerJumps (logoJump, "#opening");

   

//GRID VIEW FLIP CARDS FOR REPSONSIVE - https://3dtransforms.desandro.com/card-flip.//
...
cardClosing.forEach(function(cardClosing){
  cardClosing.addEventListener( 'click', function() {
    document.querySelector(".inq").scrollIntoView({ block: 'end',  behavior: 'smooth' });
  });
});
